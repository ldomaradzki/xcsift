...
- **MCP proxy** — Wrap Xcode's built-in MCP server so agents get structured results (`xcsift mcp`)
...
# Wrap Xcode's built-in MCP server so its build output arrives structured
